...
def format_args(string, f):
    ...
    for arg in arg_list:
        ...
        if len(type_and_optional) > 1:
            f.write(f"|{arg_name}|{type_and_optional}|{description}|\n")
...
def format_raises(string, f):
    ...
    for exception in exception_list:
        ...
        f.write(f"|{exception_name}|{exception_description}|\n")
...
def format_functions(_class, function_list, f):
    ...
    for function in function_list:
        ...
        f.write(f"\n---\n### `{getattr(eval(_class), function).__name__}()`\n")
...
>In the event that a key isn't found or has been changed, you can access the full JSON output with the `scryfallJson` variable (`{eval(_class).__name__}().scryfallJson`).
"""
        ...
        try:
            ...
            if not os.path.exists(f"./docs/{subpackage.__name__}"):
                os.makedirs(f"./docs/{subpackage.__name__}")

            with open(f"./docs/{subpackage.__name__}/{_class}.md", "w") as f:
                f.write(f"# **class** `{subpackage.__name__}.{_class}()`\n")
                f.write(intro)
                format_args(args, f)
